...
class DeviceManager extends Cubit<Map> {
  DeviceManager()
      : super({
          "#": [1, 2, 3],
          "debug_text": "default message",
          "device": ["", "", ""],
          "status": ["", "", ""]
        });
...
//Handles all incoming datagram packets until a certain interval where no new datagrams are detected
    while (true) {
      ...
      if (dg == null) {
        emit({
          'debug_text': "Timeout! No new incoming datagrams",
          "#": this.state["#"],
          "device": this.state["device"],
          "status": this.state["status"]
        });
      ...
      if (dg != null) {
        ...
        if (splitdata[0] == 'peace and long life') {
          emit({
            'debug_text': "discovered device " + dg.address.toString(),
            "#": this.state["#"],
            "device": this.state["device"],
            "status": this.state["status"]
          });
    ...
    if (this.numDevices > 0) {
      emit({
        "debug_text": "there are ${this.numDevices} devices!",
        "#": this.state["#"],
        "device": this.state["device"],
        "status": this.state["status"]
      });
      ...
      emit({
        "debug_text": "no devices discovered!",
        "#": this.state["#"],
        "device": this.state["device"],
        "status": this.state["status"]
      });
  ...
  void tabulateDevices() {
    ...
    if (this.numDevices > 0) {
      ...
      for (var pimatrix in this.deviceList) {
        ...
        temp_map["device"][num] = pimatrix.getHostname();
        temp_map["status"][num] = pimatrix.status;
        num = num + 1;
      }
    } else {
      ...
      case "rec2sd":
        {
          if (this.managerBusy == true) {
            ...
            signal = "";
            emit({
              "debug_text": "invalid action! Devices busy!",
              "#": this.state["#"],
              "device": ["", "", ""],
              "status": ["", "", ""]
            });
